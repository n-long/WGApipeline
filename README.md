...
I prefer [progressiveCactus](https://github.com/glennhickey/progressiveCactus) for the alignment task, since it is built on the existing work of LastZ and Pecan and allows for rearrangement inference. Even though it is version 0.0, it has already been used in [publications](https://scholar.google.com/scholar?hl=en&q=progressivecactus&btnG=&as_sdt=1%2C44&as_sdtp=) and has not thrown any runtime errors even using whole genome sequence. The accompanying [HALtools](https://github.com/glennhickey/hal/blob/master/README.md) can detect multiple mutation types (inversions, duplications, deletions, etc.) and includes support for detecting constrained elements (PhyloP). The rest of this document assumes you have progressiveCactus and HALtools installed and in your path. 
